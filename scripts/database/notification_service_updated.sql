...
-- 插入示例通知记录（用于演示和测试）
INSERT INTO `notifications` (`request_id`, `template_code`, `channel_code`, `provider_code`, `recipient_type`, `recipient_id`, `recipient_info`, `template_params`, `rendered_subject`, `rendered_content`, `send_status`, `sent_at`, `create_time`) VALUES
...
-- 插入示例站内信（对应上面的通知记录）
INSERT INTO `user_in_app_messages` (`notification_id`, `user_id`, `subject`, `content`, `is_read`, `create_time`) VALUES
...
INSERT INTO `user_in_app_messages` (`notification_id`, `user_id`, `subject`, `content`, `is_read`, `create_time`)
...
CREATE INDEX `idx_notifications_status_time` ON `notifications` (`send_status`, `create_time`);
CREATE INDEX `idx_notifications_template_time` ON `notifications` (`template_code`, `create_time`);
CREATE INDEX `idx_notifications_channel_time` ON `notifications` (`channel_code`, `create_time`);
CREATE INDEX `idx_notifications_recipient_type` ON `notifications` (`recipient_type`, `recipient_id`);

-- 为user_in_app_messages表创建复合索引
CREATE INDEX `idx_user_messages_user_read` ON `user_in_app_messages` (`user_id`, `is_read`, `create_time`);
CREATE INDEX `idx_user_messages_user_time` ON `user_in_app_messages` (`user_id`, `create_time` DESC);
